...
public class toolboxUtil extends AbstractPage {
    ...
    public static LoginDetails datatableLoginDetails;
    public static CoustomerDetails datatableCoustomerDetails;
...
//    }
    ...
    public void login(String credencial) {
        ...
        softly.assertThat(loginPage.loginButton().isDisplayed()).isTrue();
        softly.assertThat(loginPage.loginButton().isDisplayed()).isFalse();
        softly.assertAll();
    }

    public void newlogin(List<LoginDetails> LoginDetaillist) {
        datatableLoginDetails = Iterables.getLast(LoginDetaillist);
        waitForMoreTime();
        loginPage.emailTextFiled().sendKeys(datatableLoginDetails.getUsername());
        loginPage.passwordTextFiled().sendKeys(datatableLoginDetails.getPassword());
        loginPage.loginButton().click();
        SoftAssertions softly = new SoftAssertions();
    ...
    public void newgetStarted(List<CoustomerDetails> CoustomerDetails) {
        ...
        waitForMoreTime();
        waitForMoreTime();
        datatableCoustomerDetails = Iterables.getLast(CoustomerDetails);

        new Select(getStartedPage.selectTitle()).selectByVisibleText(datatableCoustomerDetails.getPersonaldetails().split(",")[0]);
        getStartedPage.firstnameTextfield().sendKeys(datatableCoustomerDetails.getPersonaldetails().split(",")[1]);
        getStartedPage.lastnamenameTextfield().sendKeys(datatableCoustomerDetails.getPersonaldetails().split(",")[2]);
        getStartedPage.ukbasedRadioYes().click();
        new Select(getStartedPage.businessTypeSelect()).selectByVisibleText(datatableCoustomerDetails.getEntity().split(",")[1]);
        getStartedPage.nextButton().click();
        waitForPageLoad();
        waitForMoreTime();
        businessContactPage.businessCalledTextbox().sendKeys(datatableCoustomerDetails.getBusiness_contactdetails().split(",")[0]);
//        System.out.println(businessDetails.split(",")[1]);
        if ((datatableCoustomerDetails.getBusiness_contactdetails().split(",")[1]).equals("Yes")) {
            businessContactPage.radioHasothertradingnameYes().click();
        } else if ((datatableCoustomerDetails.getBusiness_contactdetails().split(",")[1]).equals("No")) {
            businessContactPage.radioHasothertradingnameNo().click();
        }
        businessContactPage.buildingNameandNumber().sendKeys(datatableCoustomerDetails.getBusiness_contactdetails().split(",")[2]);
        businessContactPage.postCode().sendKeys(datatableCoustomerDetails.getBusiness_contactdetails().split(",")[3]);
